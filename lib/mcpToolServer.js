// Shared tool definitions + handlers for the 9 pulse-mcp tools, factored out
// so both index.js (local stdio, single-tenant via env vars) and
// hosted-server.js (remote HTTP, multi-tenant via per-request headers) can
// build an MCP Server from the same logic without duplicating it.
...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      "Checks whether the mobile bridge is reachable and returns a friendly human-readable connection summary plus the raw device status. In hosted-relay mode this reflects whether the device is actually live right now (recently pushed a heartbeat) rather than just replaying old cached data.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "list_connected_devices",
    description:
      "Lists every device that has ever pushed status to the hosted relay, with when it was last seen and whether it currently looks online. Only available when running against a hosted relay (not in direct-to-phone local mode) - use this to discover a new device's ID instead of guessing one.",
    inputSchema: { type: "object", properties: {} },
  },
  {
...
/**
 * Builds an MCP Server implementing all 9 pulse-mcp tools.
 *
 * @param {object} opts
 * @param {(path: string) => Promise<unknown>} opts.fetchFromPhone - fetches a route from the phone/relay
 * @param {() => string} opts.unreachableMessage - human-readable message when fetchFromPhone throws
 * @param {() => string} [opts.getTodayGitCommits] - returns today's git log, or a fallback message
 * @param {() => Promise<Array<{deviceId: string, lastSeenAt: string, online: boolean}>>} [opts.fetchDeviceList] - lists all devices known to the relay; omit in local (non-relay) mode
 * @param {string} [opts.name] - MCP server name
 */
export function createPulseServer({ fetchFromPhone, unreachableMessage, getTodayGitCommits, fetchDeviceList, name = "pulse-mcp" }) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      if (toolName === "get_mobile_device_status") {
        const status = await fetchFromPhone("/status");
        return { content: [{ type: "text", text: JSON.stringify(status, null, 2) }] };
      }

      if (toolName === "get_mobile_app_logs") {
        const { logs } = await fetchFromPhone("/logs");
        const filtered = args.level ? logs.filter((entry) => entry.level === args.level) : logs;
        return { content: [{ type: "text", text: JSON.stringify({ logs: filtered }, null, 2) }] };
      }

      if (toolName === "check_mobile_connection") {
        const status = await fetchFromPhone("/status");
        const platformLabel =
          status.platform === "ios" ? "iOS" : status.platform === "android" ? "Android" : status.platform;
        // `online` is only present in hosted-relay mode (see relay-server/server.js); a
        // successful direct-to-phone fetch in local mode means it's live by definition.
        const isOnline = status.online !== false;
        const icon = isOnline ? "✅" : "⚠️";
        const verb = isOnline ? "Connected to" : "No live heartbeat from";
        const staleNote = isOnline ? "" : ` (last seen ${status.lastSeenAt} — showing its last known state, not a live read)`;
        const summary = `${icon} ${verb} ${platformLabel} device (${platformLabel} ${status.osVersion}) — app v${status.appVersion} (build ${status.buildNumber}), battery ${status.batteryLevel}, screen: ${status.activeRoute}${staleNote}`;
        return {
          content: [
      ...
      if (toolName === "list_connected_devices") {
        if (!fetchDeviceList) {
          return {
            content: [
              { type: "text", text: "Device listing is only available in hosted-relay mode (PULSE_RELAY_URL not configured for this connection)." },
            ],
          };
        }
        const devices = await fetchDeviceList();
        if (!devices || devices.length === 0) {
          return { content: [{ type: "text", text: "No devices have pushed status to the relay yet." }] };
        }
        return { content: [{ type: "text", text: JSON.stringify({ devices }, null, 2) }] };
      }

      if (toolName === "diagnose_mobile_error") {
        ...
        if (!latestCrash && !latestErrorLog) {
          return {
            content: [
              { type: "text", text: "Nothing to diagnose yet. No crash was captured and no log entry with level 'error' was found on the device." },
            ],
          };
        }
      ...
      if (toolName === "get_bug_report") {
        ...
        const looksLikeFailure = /error|fail|fatal|exception/i.test(lastStep.description);
      ...
      if (toolName === "get_saved_bug_reports") {
        const { reports } = await fetchFromPhone("/reports");
        if (!reports || reports.length === 0) {
          return {
            content: [{ type: "text", text: "No saved bug reports yet. Reports are auto-saved every time 'Stop Recording' is tapped in the app." }],
          };
        }
        return { content: [{ type: "text", text: JSON.stringify({ reports }, null, 2) }] };
      }

      if (toolName === "get_mobile_crash_logs") {
        const { crashes } = await fetchFromPhone("/crashes");
        if (!crashes || crashes.length === 0) {
          return {
            content: [
              { type: "text", text: "No crashes captured. The app hasn't hit an uncaught exception, render error, or unhandled promise rejection since it started (or since the last time crashes were flushed)." },
            ],
          };
        }
        const categorized = crashes.map((c) => ({ ...c, ...categorizeErrorMessage(c.message || "") }));
        return { content: [{ type: "text", text: JSON.stringify({ crashes: categorized }, null, 2) }] };
      }

      if (toolName === "get_standup_snapshot") {
        const status = await fetchFromPhone("/status");
        const commits = getTodayGitCommits ? getTodayGitCommits() : "Not available in this mode.";
        return { content: [{ type: "text", text: JSON.stringify({ mobileDeviceStatus: status, todayGitCommits: commits }, null, 2) }] };
      }
      ...
      return {
        isError: true,
        content: [{ type: "text", text: unreachableMessage() }],
      };
    }
  });
